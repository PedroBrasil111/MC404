...
int binary_packed_value(int* values) {
  ...
  int mask, ret = 0, start_bit = 0;
  for (int i = 0; i < 5; i++) {
    mask = mask_bits(num_bits[i]);
    values[i] &= mask; // sobram os bits desejados 
    ret = ret | (values[i] << start_bit); // desloca e faz a uniao
...
/* Constroi o array de inteiros values com os valores inputados em input */
