...
.set MIDI_CH, 0xFFFF0300   # unsigned byte
.set MIDI_INST, 0xFFFF0302 # short
.set MIDI_NOTE, 0xFFFF0304 # unsigned byte
.set MIDI_VEL, 0xFFFF0305  # unsigned byte
.set MIDI_DUR, 0xFFFF0306  # unsigned short

# Plays a note in the specified channel
# Parameters: a0 - Channel, a1 - Instrument ID, a2 - Note,
#             a3 - Note velocity, a4 - Note duration
.globl play_note
play_note:
    li t0, MIDI_INST
    ...
    li t0, MIDI_CH
    sb a0, (t0)       # Start playing note on ch
    ret

# Interrupt service routine. Treats the interrupt generated by the GPT
isr:
    # Storing registers
    ...
    sw t0, (t1)             # Set next interrupt to happen in 100 ms
    # Loading registers
    lw t0, 0(sp)
    lw t1, 4(sp)
    addi sp, sp, 16
